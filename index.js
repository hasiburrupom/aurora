...
bot.on("ready", () =>{
  ...
  bot.user.setActivity("Aurora_Ar  🕊", {type: "WATCHING"})
});
...
> \`invite/inv\` - \`invite link for the bot\``
  ...
  if (command === "invite" || command === "inv") {
    return message.channel.send({
        embed: {
          color: "BLUE",
          description: "https://discord.com/api/oauth2/authorize?client_id=820708958258987019&permissions=37092928&scope=bot"
        }
    });
  }
  ...
  if (command === "play" || command === "p" || command === "") {
    ...
    if (!permissions.has("CONNECT")) {
      return message.channel.send({
        embed: {
          color: "RED",
          description:
            "Aurora Sorry, but I need a **`CONNECT`** permission to proceed!"
        }
      });
    }
    if (!permissions.has("SPEAK")) {
      return message.channel.send({
        embed: {
          color: "RED",
          description: "Aurora Sorry, but I need a **`SPEAK`** permission to proceed!"
        }
      });
    }
    if (!url || !searchString)
      return message.channel.send({
        embed: {
          color: "RED",
          description: "Aurora Please input link/title to play music"
        }
      });
  ...
  if (command === "search" || command === "sc") {
    ...
    if (!voiceChannel)
      return message.channel.send({
        embed: {
          color: "RED",
          description:
            "Aurora I'm sorry, but you need to be in a voice channel to play a music!"
        }
      });
    const permissions = voiceChannel.permissionsFor(message.client.user);
    if (!permissions.has("CONNECT")) {
    ...
    if (!url || !searchString)
      return message.channel.send({
        embed: {
          color: "RED",
          description: "Aurora Please input link/title to search music"
        }
      });
    if (url.match(/^https?:\/\/(www.youtube.com|youtube.com)\/playlist(.*)$/)) {
      const playlist = await youtube.getPlaylist(url);
      ...
      try {
        ...
        try {
          ...
          let embedPlay = new MessageEmbed()
            ...
            .setFooter(
              "Aurora Please choose one of the following 10 results, this embed will auto-deleted in 15 seconds"
            );
          ...
          try {
            ...
            return message.channel.send({
              embed: {
                color: "RED",
                description:
                  "Aurora The song selection time has expired in 15 seconds, the request has been canceled."
              }
            });
          }
    ...
    if (!message.member.voice.channel)
      return message.channel.send({
        embed: {
          color: "RED",
          description:
            "Aurora I'm sorry, but you need to be in a voice channel to skip a music!"
        }
      });
    if (!serverQueue)
      return message.channel.send({
        embed: {
          color: "RED",
          description: "Aurora There is nothing playing that I could skip for you"
        }
      });
    ...
    if (!message.member.voice.channel)
      return message.channel.send({
        embed: {
          color: "RED",
          description:
            "Aurora I'm sorry but you need to be in a voice channel to play music!"
        }
      });
    if (!serverQueue)
      return message.channel.send({
        embed: {
          color: "RED",
          description: "Aurora, There is nothing playing that I could stop for you"
        }
      });
    serverQueue.songs = [];
    serverQueue.connection.dispatcher.end(
    ...
    if (!message.member.voice.channel)
      return message.channel.send({
        embed: {
          color: "RED",
          description:
            "Aurora, I'm sorry, but you need to be in a voice channel to set a volume!"
        }
      });
    if (!serverQueue)
      return message.channel.send({
        embed: {
          color: "RED",
          description: "Aurora, There is nothing playing"
        }
      });
    if (!args[1])
      return message.channel.send({
    ...
    if (isNaN(args[1]) || args[1] > 100)
      return message.channel.send({
        embed: {
          color: "RED",
          description:
            "Aurora, Volume only can be set in a range of **`1`** - **`100`**"
        }
      });
